...
Advert.prototype.initializeHTML = function initializeHTML() {
  ...
  if (this.app.options.display_adverts == 0) { 
alert("No Display Adverts Options Selected");
return; 
  }
...
Advert.prototype.fetchAdvert = function fetchAdvert(width, height) {
  ...
  console.log("FETCH THIS ADVERT:" + '/advert/'+this.app.wallet.returnPublicKey()+'/'+width+'/'+height);
...
console.log(psql + " -- " + pparams);
    advert_self.app.storage.queryDatabase(psql, pparams, function(perr, prow) {

      if (perr != null) {
        console.log(JSON.stringify(perr) + " -- " + JSON.stringify(prow));
	return;
      }
...
console.log(sql);
console.log(req.params.publickey +" --- " + 0);
