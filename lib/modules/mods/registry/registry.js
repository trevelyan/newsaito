...
Registry.prototype.installModule = async function installModule() {
  ...
  registry_self.db = await sqlite.open('./data/registry.sq3');
  ...
  let res = await registry_self.db.run(sql, {});
...
Registry.prototype.attachEvents = function attachEvents(app) {
  ...
  $('#submit').on('click', function() {
    ...
    let requested_identifier = $('#requested_identifier').val();
alert("HERE: " + requested_identifier);
    app.dns.fetchPublicKey(requested_identifier, function(answer) {

      let obj = JSON.parse(answer);
      //if (obj.err != "") {

      //  alert("That email address appears to be taken, please try another.");

      //} else {

        newtx = app.wallet.createUnsignedTransaction(this.publickey, 0, 3);
        if (newtx == null) { alert("Your wallet does not have the funds to register an email address"); }
        newtx.transaction.msg.module = this.name;
        newtx.transaction.msg.requested_identifier = requested_identifier;
        newtx = app.wallet.signTransaction(newtx);
        app.network.propagateTransaction(newtx);

	alert("Your request has been submitted. Redirecting to email client to await response...");
	location.href = "/email";
