...
console.log("THIS IS THE BLOCK: " + JSON.stringify(tmpblk.block));
    ...
    let burn_fee_needed = Big(this.app.burnfee.returnBurnFeePaidForThisBlock(tmpblk));
    ...
    let total_fees      = Big(tmpblk.returnFeesTotal()).minus(creator_surplus).minus(burn_fee_needed);
    let locked          = burn_fee_needed.plus(total_surplus).plus(tmpblk.returnCoinbase());
    let node_share2     = locked.div(2).toFixed(8);
    let miner_share2    = locked.minus(Big(node_share2)).toFixed(8);

console.log(burn_fee_needed + " --- " + locked.toFixed(8) + " -- " + node_share2 + " -- " + miner_share2);
  ...
  for (var sd = 0; sd < spent.length; sd++) {
    ...
    if (sd == 0) {
      ...
      locked_coinbase = locked_coinbase.plus(Big(coinbase[sd]));
      ...
      //
      if (gt[sd-1] != 1) {
        console.log("LOCKED COINBASE: " + coinbase[sd]);
        locked_coinbase = locked_coinbase.plus(Big(coinbase[sd]));
        node_share[sd] = "0";
        miner_share[sd] = "0";
      }
