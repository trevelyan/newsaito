...
Bank.prototype.installModule = async function installModule() {
  ...
  try {
    ...
    this.db = await sqlite.open('./data/bank.sq3');

    var sql = "CREATE TABLE IF NOT EXISTS mod_bank (id INTEGER, parent_id INTEGER, depositor_publickey TEXT, vault_publickey TEXT, vault_privatekey TEXT, unixtime INTEGER, rebroadcast INTEGER, rebroadcast_block_id INTEGER, rebroadcast_block_hash TEXT, tx TEXT, block_id INTEGER, tx_id INTEGER, slip_id INTEGER, bhash TEXT, longest_chain INTEGER, PRIMARY KEY(id ASC))";

    let res = await this.db.run(sql, {});
    res = await this.db.run("CREATE INDEX mod_bank_idx ON mod_bank(depositor_publickey, rebroadcast, longest_chain)", {});
    res = await this.db.run("CREATE INDEX mod_bank_idx2 ON mod_bank(rebroadcast, longest_chain)", {});

  } catch (err) {}

}
Bank.prototype.initialize = async function initialize() {
...
Bank.prototype.onChainReorganization = async function onChainReorganization(block_id, block_hash, lc) {
  var bank_self = this;
  ...
  try {
    let res = await bank_self.db.run(sql, params);
  } catch (err) {}
}
...
Bank.prototype.onConfirmation = async function onConfirmation(blk, tx, conf, app) {
  ...
  if (bank_req == "deposit") {
    ...
    if (conf == 0) {
	  ...
	  $bhash : dp_bhash 
        ...
        res = await bank_self.db.run(sql, params);
	...
	var params2 = { $id : deposit_id , $rbi : dp_block_id , $rbh : dp_bhash }
        let res = await bank_self.db.run(sql2, params2);
  ...
  if (conf == 0) {
    ...
    var rows = await bank_self.db.all(bank_sql, params);
    if (rows != null) {
...
Bank.prototype.onNewBlock = async function onNewBlock(blk) {
  ...
  var rows = await this.db.all(bank_sql, params);
  if (rows != null) {
    for (var x = 0; x < rows.length; x++) {
      var row = rows[x];
      ...
      if (send_amt < 0.0001) {
        ...
        let res = await bank_self.db.run(sql3, params3);
