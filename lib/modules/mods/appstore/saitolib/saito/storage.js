...
Storage.prototype.saveBlock = async function saveBlock(blk, lc = 0) {
  ...
  // save to database //
  //////////////////////
  var sql2 = `INSERT INTO blocks (block_id, golden_ticket, reindexed, block_json_id, hash, conf, longest_chain, min_tx_id, max_tx_id)
  			VALUES ($block_id, $golden_ticket, 1, $block_json_id, $hash, 0, $lc, $mintxid, $maxtxid)`;
  var params2 = {
  ...
  if (blk.saveDatabaseId > -1) {
	sql2 = `INSERT INTO blocks (id, block_id, golden_ticket, reindexed, block_json_id, hash, conf, longest_chain, min_tx_id, max_tx_id)
	    VALUES ($dbid, $block_id, $golden_ticket, 1, $block_json_id, $hash, 0, $lc, $mintxid, $maxtxid)`;
    params2 =  {
  ...
  try {
    ...
    var tmp_filepath = `${this.directory}blocks/${tmp_filename}`;
 ...
 *
 * @param {integer} how many blocks to index
 */
Storage.prototype.loadBlocksFromDisk = async function loadBlocksFromDisk(mylimit=0) {
    ...
    for (let i = 1, n = files.length; i < n; i++) {
      try {
        ...
        let this_block_id = fileID.substr(0, fileID.indexOf("-"));
        let this_db_id    = fileID.substr(fileID.indexOf("-") + 1, fileID.indexOf(".")-fileID.indexOf("-")-1);
        ...
        blk.save_bid    = this_block_id; 	// block_id
        blk.save_db_id  = this_db_id;       // id
    ...
    // after finishing what we need to
    // this.app.network.fetchBlockchain();
    this.reindexing_active = false;
    return;
}
...
Storage.prototype.openBlockByFilename = function openBlockByFilename(filename) {
  ...
  try {
	...
	storage_self.app.logger.logError("Error reading block from disk", {message: "", stack: err})
  }
}
