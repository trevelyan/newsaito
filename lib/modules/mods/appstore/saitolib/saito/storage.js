...
Storage.prototype.loadSingleBlockFromDisk = function loadSingleBlockFromDisk(bid) {
  if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) { return; }
  ...
  // TODO: no hash available so checking every file is required, very slow
  for (let i = 1, n = files.length; i < n; i++) {
    try {
      if (files[i].substr(0, files[i].indexOf("-")) == bid.toString()) {
        let fileID = files[i]
        let blk = this.openBlockByFilename(fileID);
 ...
 */
Storage.prototype.validateTransactionInputs = function validateTransactionInputs(slip_array) {
  if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) { resolve(false); }

  for (let i = 0; i < slip_array.length; i++) {
    if (slip_array[i].amt != 0) {
      if (shashmap.validate_slip(slip_array[i].returnIndex(), this.app.blockchain.returnLatestBlockId()) == 0) {
        return false;
      }
    }
  }
