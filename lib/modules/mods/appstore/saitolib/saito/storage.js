...
console.log("\n\n\nINSERTING: " + slip_map_index);
console.log(JSON.stringify(blk.transactions[b].transaction.to[bb]));
        shashmap.insert_slip(slip_map_index, -1);
console.log("CHECK: " +  shashmap.slip_value(slip_map_index));
      }
    }
  }
...
console.log("\n\n\nADD TX VALIDATION CHECK - BID is: " + bid);
  for (let i = 0; i < slip_array.length; i++) {
    if (slip_array[i].amt != 0) {

console.log("OUR SHASHMAP INDEX IS: " + slip_array[i].returnIndex());
console.log("CHECK: " +  shashmap.slip_value(slip_array[i].returnIndex()));
