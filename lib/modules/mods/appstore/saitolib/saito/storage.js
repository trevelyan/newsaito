...
console.log("SHASHMAP INSERT: " + slip_map_index);
...
Storage.prototype.loadSingleBlockFromDiskWithCallback = async function loadSingleBlockFromDiskWithCallback(hash, mycallback) {
  let blk = await loadSingleBlockFromDisk(hash);
  mycallback(blk);
}
...
Storage.prototype.loadSingleBlockFromDiskByIdWithCallback = async function loadSingleBlockFromDiskByIdWithCallback(block_id, mycallback) {
  let blk = await loadSingleBlockFromDiskById(block_id);
  mycallback(blk);
}
...
Storage.prototype.updateShashmap = function updateShashmap(slip_index, bid) {
  shashmap.insert_slip(slip_map_index, this.block.id);
}
