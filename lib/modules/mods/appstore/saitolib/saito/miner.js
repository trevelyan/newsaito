...
Miner.prototype.attemptSolution = async function attemptSolution(prevblk) {
  ...
  if (this.app.BROWSER == 1) {
    if (this.app.wallet.inputs.length < 2) {
      return;
    }
  }
...
console.log("TOTAL FEES PAID: " + total_fees_in_prevblk + " -- " + JSON.stringify(prevblk.block.bf));
console.log("CREATOR SURPLUS: " + creator_surplus);
console.log("FEES TO GT:      " + Big(total_fees_in_prevblk).minus(Big(creator_surplus)));
console.log("TOTAL GT VALUE:  " + total_fees_for_miners_and_nodes);
console.log("BURN FEE:        " + total_fees_needed_for_prevblk);
//
// we need to add the total amount
//
