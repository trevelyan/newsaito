 ...
 * @param {integer} propagate NOW instead of queueing?
 **/
  ...
  try {
    ...
    this.socket.on('request', (data, mycallback=null) => {
      ...
      if (message.request == "handshake") {
        ...
        console.log("SETTING PEER PUBLICKEY: " + message.data.publickey);
        ...
        //if (peer_last_bid > this.app.blockchain.returnLatestBlockId()) {
...
console.log("OUR LAST SHARED BID: " + last_shared_bid);

        //
	// send blockchain info
	//
console.log(this.app.blockchain.returnLatestBlockId() + " -- " + last_shared_bid);
...
console.log("handshake signature getting verified...");
	...
	if (this.handshake_requests > 5) {
      ...
      if (message.request == "missing block") {
        ...
        // note that lasthash is now "last_hash"
...
console.log("BLOCK AVAILABLE: " + message.bhash);
console.log("________________________________\n");
        if (this.app.blockchain.isHashIndexed(message.bhash) != 1) { this.app.mempool.fetchBlock(this, message.bhash); }
        return;
      }
...
console.log(JSON.stringify(blocks));
	  ...
	  let hash = this.app.crypto.hash(blocks[i].prehash + prevhash);
          prevhash = hash;
console.log("CALCULATED BID " + bid + " is " + hash);
      ...
      if (message.request == "transaction") {
        ...
        this.app.mempool.addTransaction(message.data);
...
console.log("sending blockhain...");
console.log(JSON.stringify(this.app.blockchain.index.lc));


  if (start_bid == 0) {
    let start_bid = this.app.blockchain.returnLatestBlockId() - 10;
...
console.log("start_bid = " + start_bid + " ----> " + this.app.blockchain.index.hash.length);
...
/**
 * send a request to a peer server
 *
 * @param {string} message (i.e. "block")
 * @param {string} data {i.e json object}
 **/
