...
console.log(this.sendblocks + " -- " + this.sendtransactions + " -- " + this.sendtickets);
  ...
  //
console.log("IS SOCKET NULL? ");
  if (this.socket != null) {
console.log("IS SOCKET NULL? NO it is NOT! ");
    if (this.socket.connected == true) {
console.log("EMITTING: " + JSON.stringify(userMessage));
      this.socket.emit('request',JSON.stringify(userMessage));
    } else {
console.log("QUEUEING: " + JSON.stringify(userMessage));
	...
	if (last_hash == "") {
        ...
        if (last_bid > 0) {
          ...
          if (missing_bid > last_bid+1) {
            peer_self.sendBlockchain(last_bid+1);
          } else {
...
console.log(`IS HASH INDEXED? ${this.app.blockchain.isHashIndexed(hash)}`);
=======
>>>>>>> c3da49bdf5d3b780d2c82bf13daeb6e47a4e5c4f
