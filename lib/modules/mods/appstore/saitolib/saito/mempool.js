...
Mempool.prototype.bundleBlock = async function bundleBlock() {
  ...
  if (Big(this.bundling_fees_needed).lte(Big(credits))) {
	...
	//
	  ...
	  blk.block.txsjson.push(JSON.stringify(tx));
...
Mempool.prototype.addTransaction = async function addTransaction(tx, relay_on_validate=0) {
  ...
  if (transaction_imported == 0) {
    ...
    //
    // TODO: VALIDATE THE TXS AS WE IMPORT THEM
    //

console.log("TRYING TO ADD TX INPUTS: " + this.app.storage.validateTransactionInputs(tx.transaction.from, this.app.blockchain.returnLatestBlockId()));

    //
    // TODO -- send to the transaction class to validate, with null block
    //
