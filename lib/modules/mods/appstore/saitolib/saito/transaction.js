...
Transaction.prototype.validate = function validate(app, blk=null) {
  ...
  //
  // we have done everything that requires no block
  //
  if (blk == null) { return 0; } 


  let block_paysplit_vote = blk.block.vote;
  let block_id = blk.block.id;
  let avg_fee = 2;
  ...
  ///////////////////////////
  if (!saito.crypto().verifyMessage(this.returnSignatureSource(),this.transaction.sig,this.returnSender())) {
  ...
  ////////////////////////////
  if (!saito.crypto().verifyMessage(this.returnMessageSignatureSource(),this.transaction.msig,this.returnSender())) {
