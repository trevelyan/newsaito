...
<<<<<<< HEAD
  ...
  if (this.block.txsjson.length > 0) {
    ...
    console.log(t + " -------> " + this.block.merkle);
...
=======
    ...
    for (let i = 0; i < blk.transactions.from.length; i++) {
      if (transaction_input_hmap[blk.transactions.from[i].returnIndex()] != undefined) {
        console.log("Block invalid: doublespend input: "+blk.transactions.from[i].returnIndex());
        this.app.logger.logError("Block merkle root hash is not as expected", {message:"",err:""});
        return 0;
      }
      transaction_input_hmap[blk.transactions.from[i].returnIndex()] = 1;
    }


    //
    // validate against shashmap
    //
    for (let i = 0; i < this.transactions.length; i++) {
      if (! this.storage.validateTransactionInputs(this.transactions.from, this.block.id) ) {
        console.log("Block invalid: input slip did not shashmap validate -- attempted doublespend");
        this.app.logger.logError("Block invalid: input slip did not shashmap validate -- attempted doublespend", {message:"",err:""});
        return 0;
      }
    }
...
>>>>>>> 5279e89a6ba92662620d4c44422e697dc641dff0
    }
  }
...
console.log("BFN: " + burn_fee_needed.toFixed(8) + " while credits are: " + credits_available.toFixed(8) + " --> " + surplus_available.toFixed(8));
...
<<<<<<< HEAD
    ...
    if (surplus_available.gt(0)) {
      let feetx = this.returnFeeTransaction();
=======
      //////////////////////////////
      // validate fee transaction //
      //////////////////////////////
...
>>>>>>> 5279e89a6ba92662620d4c44422e697dc641dff0
	...
	if (feetx_count > 1) {
      ...
      let v = Big(0);
      for (let i = 0; i < feetx.transaction.to.length; i++) {
        v = v.plus(feetx.transaction.to[i].amt);
      }

<<<<<<< HEAD
console.log("IS TRUE: " + v.eq(surplus_available));
      if (v.eq(surplus_available) == false) {
...
>>>>>>> 5279e89a6ba92662620d4c44422e697dc641dff0
    ...
    if (gttx == null) {
      ...
      if (!this.app.goldenticket.validateSolution(this, gttx.transaction.from[0].add, gttx.transaction.msg)) {
