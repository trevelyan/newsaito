...
console.log("\n\n\nWE HAVE FOUND A TX TO REBROADCAST: ");
...
console.log("CANNOT FIND TX IN BLOCK!");
  ...
  for (let v = 0; v < this.transactions.length; v++) {
	...
	//console.log(JSON.stringify(this.transactions[v].transaction));
        total_rebroadcast++;
      }
...
Block.prototype.validate = async function validate() {
  ...
  if ((this.block.id - this.app.blockchain.genesis_period) > this.app.blockchain.lowest_acceptable_bid) {
