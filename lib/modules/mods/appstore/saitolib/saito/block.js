...
Block.prototype.calculateReclaimedFunds = async function calculateReclaimedFunds() {
  return new Promise(async (resolve, reject) => {
    ...
    for (var i = 0; i < blk.transactions.length; i++) {
      ...
      if (blk.transactions[j].isAutomaticallyRebroadcast()) {	
	...
	//
        // go through the transactions, one by one to check 
	//
        for (var j = 0; j < blk.transactions[j].transaction.to.length; j++) {

          var slip       = blk.transactions[i].transaction.to[j];
    ...
    resolve({reclaimed: unspent_amt.toFixed(8), validates: true});
  });
}
