...
Email.prototype.initializeHTML = function initializeHTML(app) {
  ...
  selobj.empty();
console.log("\n\n\nLENGTH OF MODULES: " + email_self.app.modules.mods.length);
  for (c = 0; c < email_self.app.modules.mods.length; c++) {
console.log("EMAIL MODULE INIT: " + c);
    if (email_self.app.modules.mods[c].handlesEmail == 1) {
console.log("IS EMAIL BROWSER ACTIVE: " + email_self.browser_active );
      if ( email_self.browser_active == 1 ) {
console.log("IS EMAIL BROWSER ACTIVE2: " + email_self.browser_active );
    ...
    $('#send').on('click', function() {
      ...
      if (to.indexOf("@saito") > 0) {
        ...
        if (pk == "") {
          ...
          // send email using local publickey
console.log("TESTING SEND EMAil 2!");
          to = pk;
console.log("TESTING SEND EMAil 3: " + to + " -- " + amount + " -- " + fee);
          newtx = app.wallet.createUnsignedTransaction(to, amount, fee);
console.log("TESTING SEND EMAil4!");
          if (newtx == null) { console.log("NULL"); return; }
console.log("TESTING SEND EMAil4!");
          modsel = email_self.active_module;
          newtx = app.modules.formatEmailTransaction(newtx, modsel);
          newtx = app.wallet.signTransaction(newtx);
          email_self.showEmailAlert("<span id='sendingthismsg'>attempting to broadcast transaction....</span>");
console.log("TESTING SEND EMAil5!");
          app.network.propagateTransactionWithCallback(newtx, function() {
...
console.log("TESTING SEND EMAil A!");
        var newtx = app.wallet.createUnsignedTransaction(to, amount, fee);

        if (newtx == null) { 
console.log(JSON.stringify(email_self.app.wallet.wallet));
console.log("TX is null!");
return; 

        }

console.log("TESTING SEND EMAil B!");

        modsel = email_self.active_module;

        newtx = app.modules.formatEmailTransaction(newtx, modsel);
console.log("TESTING SEND EMAil C!");
        newtx = app.wallet.signTransaction(newtx);

        email_self.showEmailAlert("<span id='sendingthismsg'>attempting to broadcast transaction....</span>");
console.log("TESTING SEND EMAil D!");
        app.network.propagateTransactionWithCallback(newtx, function() {

console.log("TESTING SEND EMAil E!");
          email_self.showBrowserAlert("your message has been broadcast to the network");
          email_self.closeMessage();
console.log("TESTING SEND EMAil F!");
        });
      }
...
console.log("AMTI 1: "+tx.transaction.msg.title);
        app.modules.returnModule("Email").addMessageToInbox(tx, app);
      }
    } else {
      if (app.BROWSER == 1) {
        if (tx.transaction.to[0].add == app.wallet.returnPublicKey()) {
console.log("AMTI 2: "+tx.transaction.msg.title);
          app.modules.returnModule("Email").addMessageToInbox(tx, app);
          app.archives.saveTransaction(tx);
        }
      }
