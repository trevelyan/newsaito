...
console.log("fn: " + this.bundling_fees_needed + " -- " + credits);
console.log("block lc: " + this.app.blockchain.lc);
if (prevblk != null) {
        ...
        for (let i = 0; i < this.transactions.length; i++) {
          ...
          if (this.transactions[i].transaction.type == 1) {
            ...
            if (this.transactions[i].transaction.msg.target != prevblk.returnHash()) {
              console.log("GTX IS INCORRECT FOR BLOCK");
              addtx = 0;
            }
          }
          if (blk.block.vote == -1) {
...
if (surplus_fees.lt(0)) {
  ...
  process.exit(0);
        ...
        // propagate our block
        //
console.log("\n\n\nPROPAGATING THIS NEW BLOCK!");
        this.app.network.propagateBlock(blk);
console.log("PROPAGATING THIS NEW BLOCK!\n\n");
...
Mempool.prototype.returnAvailableFees = function returnAvailableFees(vote=0, prevblk_hash) {
  ...
  var v = Big(0);
  for (let i = 0; i < this.transactions.length; i++) {
    // need to verify that for a GTX we're assigned to the right block before calculating
    // TODO:
    if (this.transactions[i].transaction.type == 1) {
      if (this.transactions[i].transaction.msg.target != prevblk_hash) { return v; }
    }
    switch(vote) {
      case -1:
