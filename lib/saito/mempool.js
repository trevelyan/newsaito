...
/**
 * Pops a block from the beginning of the block mempool queue
 * @returns {saito.block} tmpblk
 */
Mempool.prototype.dequeueBlock = function returnBlock() {
  if (this.blocks.length == 0) { return null; }
  return this.blocks.shift();
}

/**
 * Requeues a block into the block mempool queue
...
Mempool.prototype.processBlocks = function processBlocks() {
  ...
  if (this.processing_timer == null) {
    this.processing_timer = setInterval(function() {
      ...
      //
      if (mempool_self.app.monitor.canBlockchainAddBlockToBlockchain()) {
        var blk = mempool_self.dequeueBlock();
        mempool_self.app.blockchain.addBlockToBlockchain(blk)
      }
