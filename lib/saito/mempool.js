...
console.log(" .. .adding to mempool: " + bhash);
...
console.log(" .. .added to mempool queue: " + bhash);
...
console.log("1 " + this.downloads.length);
...
console.log("2 " + this.downloading_active);
      if (this.downloading_active == 1) { return; }
      this.downloading_active = 1;

console.log(" now downloading is active .. ");
...
console.log("DLING: " + block_to_download_url);

          $.get(
            block_to_download_url, {
            }, (data) => {
console.log("BLK is: " + data);
...
Mempool.prototype.addBlock = function addBlock(blk) {
  ...
  //
  // this is where blocks are handed to us that come in 
  // over the mempool, so we want to handle superficial 
  // validation and propagation here. (???)
  //
...
Mempool.prototype.bundleBlock = async function bundleBlock() {
  ...
  if (prevblk != null) {
    ...
    console.log(`${new Date()} : ${this.app.burnfee.returnBurnFeeNeeded(prevblk, ((new Date().getTime()) - prevblk.block.ts)).toFixed(8)} ---- ${credits} ( ${this.transactions.length} / ${this.containsGoldenTicket()} / ${this.app.wallet.returnBalance() } )`);
  }
    ...
    if (
      ...
      try {
        ...
        if (prevblk != null) {
          ...
          let total_fees_needed = this.app.burnfee.returnBurnFeeNeeded(prevblk, (blk.block.ts - prevblk.block.ts));
          ...
          console.log("pushing new fee-capture tx into block... ");
...
Mempool.prototype.addTransaction = async function addTransaction(tx, relay_on_validate=0) {
  ...
  if (transaction_imported == 0) {
    ...
    //if (this.app.storage.validateTransactionInputs(tx.transaction.from, this.app.blockchain.returnLatestBlockId())) {
