...
Mempool.prototype.bundleBlock = async function bundleBlock() {
  ...
  // ensure not bundling
  if (this.app.monitor.canMempoolBundleBlock()) { return; }
  ...
  //console.log(`${new Date()} : ${fees_needed.toFixed(8)} ---- ${credits.toFixed(8)}  ${mempool_self.bundling_fees_needed} (${mempool_self.transactions.length}/${mempool_self.returnNormalTransactionsInMempool()} -- ${mempool_self.app.wallet.returnBalance()})`);
