...
Mempool.prototype.bundleBlock = function bundleBlock(previousBlock) {
  ...
  //
  if (this.currently_creating == true) {
    return;
  }
  this.currently_creating = true;
  ...
  this.adjustBurnFee(this.burnfee, this.blockTime);
...
Mempool.prototype.processBlocks = function processBlocks() {
  ...
  if (this.currently_processing) {
    this.app.logger.logInfo(`Mempool processing.... not adding new block to blockchain`);
    return;
  }

  if (this.blocks.length == 0) {
    this.app.logger.logInfo(`Mempool processing.... no blocks to add to blockchain`);
    this.currently_processing = false;
    return;
  }

  if (this.processing_timer == null) {
    if (this.currently_clearing && this.currently_processing) { return; }

    this.currently_blocks = true;
    ...
    if (blk != null) {
      ...
      if (blk.prevalidated == 0) {
        delete_blk_from_mempool = this.app.blockchain.addBlockToBlockchain(blk);
      } else {
        delete_blk_from_mempool = this.app.blockchain.addBlockToBlockchain(blk, "force");
      }
      if (delete_blk_from_mempool == 1) {
        this.clearMempool(blk);
      }
    }
...
Mempool.prototype.adjustBurnFee = function adjustBurnFee(bf, blockTime) {
