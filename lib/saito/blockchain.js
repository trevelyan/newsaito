...
Blockchain.prototype.addBlockToBlockchainSuccess = async function addBlockToBlockchainSuccess(newblock, pos, i_am_the_longest_chain, force) {
  ...
  // update newblock
  //
  console.log("WE'RE IN THE FUNCTION ADD BLOCK 2 BCHAIN SUCESS");
  if (i_am_the_longest_chain == 1) {
...
Blockchain.prototype.unwindChain = async function unwindChain(
  ...
  if (old_block_hashes.length > 0) {
    ...
    if (current_unwind_index == 0) {
      this.windChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, 0, resetting_flag);
    } else {
      this.unwindChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, current_unwind_index-1, resetting_flag);
    }
  } else {
...
console.log("d");
      ...
      if (current_wind_index == 0) {
        ...
        //
        if (old_block_hashes.length > 0) {
          this.windChain(newblock, pos, shared_ancestor_pos, old_block_idxs, old_block_hashes, old_block_ids, null, null, null, i_am_the_longest_chain, force, 0, 1);
        } else {
          this.addBlockToBlockchainFailure(newblock, pos, i_am_the_longest_chain, force);
        }
      } else {
        ...
        //
        this.unwindChain(newblock, pos, shared_ancestor_pos, old_block_idxs, old_block_hashes, old_block_ids, chain_to_unwind_idxs, chain_to_unwind_hashes, chain_to_unwind_ids, i_am_the_longest_chain, force, chain_to_unwind_hashes.length, 1);
