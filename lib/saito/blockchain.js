 ...
 * @param {integer} position of block in indexes
 * @param {integer} is block in longest chain
 * @param {boolean} true if added from disk
 */
 ...
 */
Blockchain.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force) {
  ...
  if (old_block_hashes.length > 0) {
    this.unwindChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, old_block_hashes.length-1, 0);
  } else {
    this.windChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, 0, 0);
  }
};
 ...
 */
Blockchain.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, current_unwind_index, resetting_flag) {
 ...
 *
 * @param {saito.block} newest block
 * @param {interger} newest block index in blockchain.index
 * @param {integer} block index of last-common-ancestor blockchain.index
 * @param {array} array of block indexes for new blocks becoming longest chain
 * @param {array} array of block hashes for new blocks becoming longest chain
 * @param {array} array of block_ids for new blocks becoming longest chain
 * @param {array} array of block indexes for existing blocks losing longest chain
 * @param {array} array of block hashes for existing blocks losing longest chain
 * @param {array} array of block_ids for existing blocks losing longest chain
 * @param {integer} is newest block longest chain (should be 1)
 * @param {string} "force" if block is loaded from disk
 * @param {integer} position in above old_block_* arrays of this block
 * @param {integer} 1 if we are unwinding a bad new chain
 **/
Blockchain.prototype.windChain = function windChain(newblock, pos, shared_ancestor_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, force, current_wind_index, resetting_flag) {
