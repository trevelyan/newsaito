...
Blockchain.prototype.returnLatestBlock = function returnLatestBlock() {
  return this.returnPreviousBlock();
}
...
Blockchain.prototype.initialize = function initialize() {
  ...
  if (this.app.options.blockchain.last_hash != undefined) {
    this.last_hash = this.app.options.blockchain.last_hash;
  }
  if (this.app.options.blockchain.last_bid != undefined) {
    this.last_bid = this.app.options.blockchain.last_bid;
  }
  if (this.app.options.blockchain.last_ts != undefined) {
    this.last_ts = this.app.options.blockchain.last_ts;
  }
  if (this.app.options.blockchain.genesis_bid != undefined) {
    this.genesis_bid = this.app.options.blockchain.genesis_bid;
  }
  if (this.app.options.blockchain.genesis_ts != undefined) {
    this.genesis_ts = this.app.options.blockchain.genesis_ts;
  }

  // 
  // do we need the fork guard?
  //
 ...
 */
Blockchain.prototype.addBlockToBlockchain = function addBlockToBlockchain(newblock=null, force=false) {
...
console.log("6: is lc? " + i_am_the_longest_chain);
  ...
  if (i_am_the_longest_chain == 1) {
    ...
    //
console.log("I am the longest Chain: " + bid + " -- " + hash);
