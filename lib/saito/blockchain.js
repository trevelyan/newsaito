...
Blockchain.prototype.addBlockToBlockchain = async function addBlockToBlockchain(newblock=null, force=false) {
  ...
  //
    ...
    if (bid >= this.index.bid[this.lc]) {
      ...
      //
      // edge-case where blocks are arriving in a different order
      // we should check to see if we have received block #43 first
      // and only have a single block as the longest chain where
      // we are receiving a much longer chain from a lower position
      // in the blockchain
      //
      // this is a hack, but we just want to say, "no, don't start
      // from block #43, we have X reasonably decent blocks and we
      // are just starting out, so lets treat them in order instead
      // of requiring the entire chain download.
      //
 ...
 * @param {function} mycallback
 */
Blockchain.prototype.returnBlockByHashIfContainsTransactionsWithCallback = async function returnBlockByHashIfContainsTransactionsWithCallback(hash, keylist, mycallback) {
  let blk = await this.returnBlockByHash(hash);
