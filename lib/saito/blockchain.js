...
Blockchain.prototype.addBlockToBlockchain = async function addBlockToBlockchain(newblock=null, force=false) {
  ...
  if (i_am_the_longest_chain == 1) {
    ...
    console.log("FINISHED VALIDATE LONGEST CHAIN")

  } else {
    // await
    await this.addBlockToBlockchainSuccess(newlock, pos, i_am_the_longest_chain);
    console.log("FINISHED ADD BLOCK TO BLOCKCHAIN SUCCESS");
  }
...
console.log("REORGING BLOCK: " + newblock.block.id + " -- " + newblock.returnHash() + " -- " + i_am_the_longest_chain);

    await this.app.storage.onChainReorganization(newblock.block.id, newblock.returnHash(), i_am_the_longest_chain);
    //
    // TODO: we do not want modules to onchain reorg unless
    // it is the first time we are getting this message from
    // them.
    //
    this.app.modules.onChainReorganization(newblock.block.id, newblock.returnHash(), i_am_the_longest_chain);
    //
    // TODO -- david
  ...
  console.log("RAN AFTER SAVE BLOCK");
...
//  this.app.logger.logInfo("Failed to validate LongestChain, blockchain failure");
  ...
  this.app.miner.stopMining();
  let blk = this.returnLatestBlock();
  if (blk != null) { this.app.miner.startMining(blk); }
  ...
  try {
    if (old_block_hashes.length > 0) {
      await this.unwindChain(
       ...
       console.log("RUNNING AFTER UNWINDCHAIN")
    } else {
      ...
      console.log("RUNNING AFTER WINDCHAIN")
    }
...
console.log("WIND CHAIN: " + newblock.returnHash());
...
console.log("HERE WE ARE TESTING: 1");
  //
...
console.log("HERE WE ARE TESTING: 2");
  if (blk.returnId() >= (this.genesis_bid + this.genesis_period + this.fork_guard)) {
    ...
    //
console.log("HERE WE ARE TESTING: 3");
...
console.log("HERE WE ARE TESTING: 4");
    ...
    //
console.log("HERE WE ARE TESTING: 5");
...
console.log("HERE WE ARE TESTING: 7");
...
console.log("HERE WE ARE TESTING: 8");
...
console.log("HERE WE ARE TESTING: 9");
...
console.log("HERE WE ARE TESTING: 10");
...
console.log("HERE WE ARE TESTING: 11");

  //////////////////
  // and clean up //
  //////////////////
  this.lc = this.lc - items_before_needed;
console.log("HERE WE ARE TESTING: 12");
  this.app.wallet.purgeExpiredSlips();

  //
  // deletes database
  //
console.log("HERE WE ARE TESTING: 13");
