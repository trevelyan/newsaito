...
BurnFee.prototype.returnBurnFeeNeeded = function returnBurnFeeNeeded(prevblk, elapsed_time=0) {
  ...
  let burn_fee_needed = prevblk.block.bf.start - (prevblk.block.bf.start * (elapsed_time / (this.heartbeat * 1000 * 2) ));
 ...
 */
BurnFee.prototype.canBundleBlock = function canBundleBlock(prevblk=null, credit="0.0") {
  let prevblk_ts = 0;
  if (prevblk != null) { prevblk_ts = prevblk.block.ts; }
  return (Big(credit).gte(this.returnBurnFeeNeeded(prevblk, ((new Date().getTime()) - prevblk_ts))));
}
...
BurnFee.prototype.validateBurnFee = function validateBurnFee(prevblk=null, blk=null) {
  ...
  let correct_start = prevblk.block.bf.start * Math.sqrt( (prevblk.block.ts - blk.block.ts) / this.heartbeat );
