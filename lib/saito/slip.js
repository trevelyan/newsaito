...
function Slip(add="", amt="0", type=0, bid=0, tid=0, sid=0, bhash="", lc=1, rn=-1) {
  ...
  this.add    = add;
  this.amt    = amt;
  this.type   = 0;	// 0 = normal transaction
			// 1 = golden ticket
			// 2 = fee ticket
...
/**
 * returnIndex returns the index created from the fields of the slip
 * @return {string} index
 */
Slip.prototype.returnIndex = function returnIndex() {
  return this.bid.toString() + this.tid.toString() + this.sid.toString() + this.bhash + this.amt.toString() + this.lc.toString();
}
