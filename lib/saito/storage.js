...
Storage.prototype.createDatabaseTables = async function createDatabaseTables() {
  ...
  try {
    ...
    await Promise.all([
      this.db.run("CREATE INDEX blocks_idx ON blocks (block_id, longest_chain)"),
      this.db.run("CREATE INDEX blocks_idx2 ON blocks (reindexed)"),
      this.db.run("CREATE INDEX blocks_idx3 ON blocks (hash)")
    ]);
  } catch(err) {
    console.log(err);
  }
...
Storage.prototype.saveOptions = function saveOptions() {
  ...
  if (this.app.BROWSER == 0) {
    try {
      fs.writeFileSync("options", JSON.stringify(this.app.options, null, 4));
    } catch (err) {
