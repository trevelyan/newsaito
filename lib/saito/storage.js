...
Storage.prototype.validateTransactionInput = function validateTransactionInput(slip, bid) {
  ...
  if (slip.amt > 0) {
    if (shashmap.validate_slip(slip.returnIndex(), bid) == 0) { 
      console.log("FAILURE: " + slip.amt + " => " + slip.returnIndex() + " -- " + shashmap.slip_value(slip.returnIndex()) + " -- " + shashmap.validate_slip(slip.returnIndex(), bid) + " (" + bid + ")");
      return false; 
    }
  }
...
console.log("UPDATING: " + slip_map_index + " --> " + bid);
