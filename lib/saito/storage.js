...
Storage.prototype.saveOptions = function saveOptions() {
  ...
  if (this.app.BROWSER == 0) {
    ...
    fs.writeFileSync("options", JSON.stringify(this.app.options, null, 4), function(err) {
      if (err) {
        console.log(err);
...
storage_self.app.logger.logError("Error thrown in execDatabase", {message:"", stack: err}); }
  });
