...
Storage.prototype.saveBlock = async function saveBlock(blk=null, lc=0) {
  ...
  if (res.lastID != undefined) {
    ...
    // already saved //
    ///////////////////
console.log("BLOCK IS SAVED 1");
    return false;

  }

console.log("BLOCK IS SAVED 2");
  return true;
}
...
/**
 * Load any blocks that exist on disk and force them into the
 * blockchain one-by-one. The sorting algorithm sorts initially
 * by timestamp and then by the name of the file.
 *
 * We force this function to run before moving onward in order
 * to get the full blockchain backup before accepting further
 * blocks.
 *
 * @params {integer} how many blocks to index
 *
 **/
Storage.prototype.loadBlocksFromDisk = function loadBlocksFromDisk() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (files[i] == "empty") {
      ...
      console.log("READING FILE FROM DISK: " + files[i]);

      let thisBlockId = files[i].substr(0, files[i].indexOf("-")); 
      let thisDatabaseId = files[i].substr(files[i].indexOf("-") + 1, files[i].indexOf(".")-files[i].indexOf("-")-1);
      // 
      ...
      // send block directly to blockchain
    }
  }
}
