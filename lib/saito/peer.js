    ...
    if (this.message_queue.length > 0) {
      if (this.socket != null) {
        if (this.socket.connected == true) {
          console.log("EMITING MESSAGE FROM QUEUE");
          console.log("MESSAGE QUEUE", this.message_queue[0]);
...
Peer.prototype.connect = function connect() {
  ...
  //
    ...
    console.log("PEER ADDRESS WE REACHING OUT TO", serverAddress);
...
Peer.prototype.sendRequest = function sendRequest(message, data="") {
  ...
  if (this.socket != null) {
    if (this.socket.connected == true) {
      ...
      console.log("ADDING REQUEST TO MESSAGE_QUEUE")
...
console.log("IS HASHED? " + this.app.blockchain.isHashIndexed(message.data.bhash));
