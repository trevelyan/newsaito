...
Miner.prototype.attemptSolution = function attemptSolution(prevblk) {
  ...
  let difficultyOrder = Math.floor(prevblock.returnDifficulty());
  let difficultyGrain = prevblock.returnDifficulty() % 1;

  // We are testing our generated has against the hash of the previous block.
  // th is the test hash.
  // ph is the previous hash.
  th = parseInt(hashValue.slice((0,difficultyOrder+)),16);
  ph = parseInt(prevblock.returnHash().slice((0,difficultyOrder+1)),16);
