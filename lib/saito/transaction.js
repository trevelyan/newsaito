...
console.log("X 3 - " + this.transaction.type);
  ...
  if (this.transaction.type >= 0) {
    ...
    for (i = 1; i < newtx.transaction.msg.loop; i++) {
      fee = fee.times(2);
    }
...
console.log("gen reb: " + JSON.stringify(this.transaction.to[slip_id]));

    //
    // create slips and set slip_id as we are not signing so it won't be automatically done
    //
