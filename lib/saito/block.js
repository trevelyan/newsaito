...
Block.prototype.calculateReclaimedFunds = async function calculateReclaimedFunds() {
  return new Promise(async (resolve, reject) => {
    if (this.app.SPVMODE == 1) { resolve({reclaimed: Big(0.0), validates: true}); }
    ...
    if (eliminated_block < 1) {
      ...
      var block = this.app.storage.loadSingleBlockFromDiskByID(eliminated_block_id);
    ...
    var block2 = this.app.storage.loadSingleBlockFromDiskByID(eliminated_block + 1);

    if (block2.golden_ticket == 0) {
...
Block.prototype.validate = function validate() {
  ...
  for (let i = 0; i < this.transactions.from.length; i++) {
    if (transaction_input_hmap[this.transactions.from[i].returnIndex()] != undefined) {
      console.log("Block invalid: doublespend input: "+this.transactions.from[i].returnIndex());
      this.app.logger.logError("Block merkle root hash is not as expected", {message:"",err:""});
      return 0;
    }
    transaction_input_hmap[this.transactions.from[i].returnIndex()] = 1;
  }
