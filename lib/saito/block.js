...
Block.prototype.validate = function validate() {
  return new Promise((resolve, reject) => {
    ...
    for (var zz = 0; zz < this.transactions.length; zz++) {
      if (this.transactions[zz].validate(this.app, this.block.paysplit_vote, this.block.id, this.returnAverageFee()) != 1) {
	...
	this.app.logger.logError(`hash: ${this.app.crypt.hash(JSON.stringify(this.transactions[zz]))}`, {message:"",err:""});
	this.app.logger.logError(`sig: ${this.transactions[zz].transaction.sig}`, {message:"",err:""});
	this.app.logger.logError(`msig: ${this.transactions[zz].transaction.msig}`, {message:"",err:""});
	reject(false);
	// return 0;
      }
...
/**
 * Make sure the slips on the transactions are valid
 * @param {saito.block} prevblk previous block
 * @returns {boolean} is tx valid?
 */
...
//       return 1;
	...
	if (this.transactions[bli].transaction.ft != null) {
	  ...
	  if (ftixAmount.lt(surplusfees)) {
	    ...
	    this.app.logger.logError("Block invalid: fee transaction is worth too little!",
		{message:"",err:""});
	    reject(false);
	    return 0;
	  }
