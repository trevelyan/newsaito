...
function Block(app, blkjson="", confirmations=-1) {
  ...
  if (this.transactions.length > 0) {
    ...
    ///////////////////////////
    this.transactions_bloom = BloomFilter.create(75000, 0.00001);
...
/**
 * do we have a bloom filter with transaction data?
 **/
Block.prototype.hasTransactionsBloomFilter = function hasTransactionsBloomFilter(add) {
  if (this.transactions_bloom == null) { return false; }
  return true;
}
...
console.log("Transaction Input is Valid");

          if (eblk.transactions[i].isAutomaticallyRebroadcast(eblk, this, ii)) {

            console.log("\n\nAutomatic Transaction Rebroadcasting: ");
            console.log(" ... slip " + ii + " is autorebroadcast: " + JSON.stringify(slip));
	    ...
	    //
            var newtx = eblk.transactions[i].generateRebroadcastTransaction(ii, 2);
            if (newtx == null) {
	      ...
	      process.exit(1);
            ...
            console.log("NEW GNRBTX: " + JSON.stringify(newtx));
	    ...
	    console.log(" ... adding rebroadcast tx to array");
            txarray.push(newtx);
            ...
            // slip is unspent
	    //
	    // but also not rebroadcast
            /////////////////////
console.log(" ... slip unspent but also not rebroadcast");
...
Block.prototype.validateRebroadcastTransactions = async function validateRebroadcastTransactions() {
      ...
      if (Big(slip.amt).gt(0)) {
	    ...
	    let is_tx_in_block = 0;
            for (let v = 0; v < this.transactions.length; v++) {
              if (this.transactions[v].transaction.sig == bblk.transactions[i].transaction.sig) {
                is_tx_in_block = 1;
  ...
  if (total_rebroadcast != current_rebroadcast) {
    console.log("Validation Error: unmatched rebroadcast transactions: " + total_rebroadcast + " - " + current_rebroadcast);
    return false;
  }
...
Block.prototype.validate = async function validate() {
  ...
  //
  if (! this.validateRebroadcastTransactions()) {
